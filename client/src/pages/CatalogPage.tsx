...
type RecordEntry = { kind: string; platform: string; modelId: string; source: string; origin: string | null; extensions: Record<string, unknown>; values: Record<string, unknown>; revision: string; readOnly?: boolean }
type CatalogData = { records: RecordEntry[]; status: { appliedVersion: string | null; appliedTier: string | null; generatedAt: string | null; lastSyncMs: number | null; lastError: string | null; official: unknown; history: { id: number; at_ms: number; action: string; detail_json: string }[] } }
...
export default function CatalogPage() {
  ...
  return <div className="mx-auto max-w-7xl space-y-6 p-6">
    ...
    <p className="text-sm text-muted-foreground">在同一目录查看和维护 Chat、Embeddings、Media 与 Quirks。来源 user / ai 的记录不会被官方月度同步覆盖；编辑目录不会自动创建 Provider 凭证。</p>
