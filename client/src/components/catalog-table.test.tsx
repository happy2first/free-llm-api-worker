...
it('separates provider/type columns, filters exact providers and changes sort direction', async () => {
  const container = document.createElement('div'), root = createRoot(container)
  try {
    await act(async () => root.render(<CatalogTable records={[row('test',9),row('test-cn',128)]} busy={false} onEdit={() => {}} />))
    expect(container.querySelector('[aria-label="筛选类型"]')).not.toBeNull()
    ...
    const sort = [...container.querySelectorAll('thead button')].find(b => b.textContent!.includes('上下文')) as HTMLButtonElement
